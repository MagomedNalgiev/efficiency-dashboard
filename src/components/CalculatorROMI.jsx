import { useState } from "react";
import { Line } from "react-chartjs-2";
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
  PointElement,
  LineElement,
  Title,
  Tooltip,
  Legend,
} from "chart.js";

ChartJS.register(
  CategoryScale,
  LinearScale,
  PointElement,
  LineElement,
  Title,
  Tooltip,
  Legend
);

export default function CalculatorROMI() {
  const [romiData, setRomiData] = useState([{ revenue: "", cost: "" }]);
  const [results, setResults] = useState([]);

  const handleChange = (index, field, value) => {
    const updated = [...romiData];
    updated[index][field] = value;
    setRomiData(updated);
  };

  const addRow = () => {
    setRomiData([...romiData, { revenue: "", cost: "" }]);
  };

  const calculateROMI = () => {
    const res = romiData.map(({ revenue, cost }) => {
      const r = parseFloat(revenue);
      const c = parseFloat(cost);
      if (!isNaN(r) && !isNaN(c) && c > 0) {
        return (((r - c) / c) * 100).toFixed(2); // ROMI в %
      }
      return 0;
    });
    setResults(res);
  };

  const data = {
    labels: romiData.map((_, i) => `Период ${i + 1}`),
    datasets: [
      {
        label: "ROMI (%)",
        data: results,
        borderColor: "rgb(34,197,94)",
        backgroundColor: "rgba(34,197,94,0.3)",
      },
    ],
  };

  const options = {
    plugins: {
      legend: {
        labels: { color: "#ffffff" },
      },
    },
    scales: {
      x: {
        ticks: { color: "#ffffff" },
        grid: { color: "rgba(255,255,255,0.1)" },
      },
      y: {
        ticks: { color: "#ffffff" },
        grid: { color: "rgba(255,255,255,0.1)" },
      },
    },
  };

  return (
    <div className="min-h-screen flex flex-col pt-16 px-4 md:px-16 py-8">
      <h1 className="text-5xl md:text-6xl font-extrabold text-white mb-4 flex items-center gap-2">
        ROMI калькулятор
      </h1>
      <p className="text-gray-200 text-lg md:text-xl leading-relaxed max-w-2xl mb-6">
        Укажите доход от маркетинговой активности и соответствующие расходы, чтобы рассчитать ROMI (Return on Marketing Investment).
      </p>

      <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 shadow w-full max-w-2xl mb-6">
        {romiData.map((entry, index) => (
          <div key={index} className="mb-6">
            <h3 className="text-white font-semibold mb-2">Период {index + 1}</h3>
            <label className="block mb-2 text-sm">Доход от маркетинга (₽):</label>
            <input
              type="number"
              value={entry.revenue}
              onChange={(e) => handleChange(index, "revenue", e.target.value)}
              className="w-full p-2 mb-2 rounded bg-white/5 text-white placeholder-white placeholder:text-sm focus:outline-none"
              placeholder="Например: 200000"
            />
            <label className="block mb-2 text-sm">Маркетинговые расходы (₽):</label>
            <input
              type="number"
              value={entry.cost}
              onChange={(e) => handleChange(index, "cost", e.target.value)}
              className="w-full p-2 mb-2 rounded bg-white/5 text-white placeholder-white placeholder:text-sm focus:outline-none"
              placeholder="Например: 100000"
            />
          </div>
        ))}

        <button
          onClick={addRow}
          className="bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded w-full mb-4"
        >
          ➕ Добавить период
        </button>

        <button
          onClick={calculateROMI}
          className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded w-full"
        >
          Рассчитать ROMI
        </button>
      </div>

      {results.length > 0 && (
        <div className="mt-8 w-full max-w-2xl">
          <Line data={data} options={options} />
        </div>
      )}

      <footer className="mt-12 text-gray-400 text-sm">© 2025 Metricspace</footer>
    </div>
  );
}
