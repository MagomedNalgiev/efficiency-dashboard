...
const INITIAL_DATA = [{ revenue: "", marketingCosts: "" }]

export default function CalculatorROMI() {
  const { performCalculation } = useCalculatorAccess('romi') // ДОБАВИТЬ
  ...
  const addPeriod = () => {
    addItem({ revenue: "", marketingCosts: "" })
  }
  ...
  const calculateROMI = () => {
    const result = performCalculation(() => {
      return periods.map(period => {
        ...
        const costs = parseFloat(period.marketingCosts) || 0
        if (costs > 0) {
  ...
  const options = {
    ...
    scales: {
      ...
      y: {
        ...
        title: { display: true, text: 'ROMI (%)', color: '#ffffff' }
      },
    },
  }

  return (
    <AccessGate calculatorId="romi"> {/* ДОБАВИТЬ AccessGate */}
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-white/80 mb-8">
              ...
              Укажите выручку и маркетинговые расходы для каждого периода.
            </p>
              ...
              {periods.map((period, index) => (
                <div key={index} className="mb-6 p-4 bg-white/5 rounded-lg">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-white mb-2">Выручка от маркетинга (₽):</label>
                      <input
                        type="number"
                        min="0"
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6">
                <div className="mb-4 text-center">
                  ...
                  <p className="text-white/80">
                    Средний ROMI: <span className={`font-bold ${
                      parseFloat(avgROMI) > 0 ? 'text-green-400' : 'text-red-400'
                    }`}>{avgROMI}%</span>
                  </p>
                </div>
